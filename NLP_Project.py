...
# 2. VERY simple NER stub – replace with your Week-3/BioBERT NER
def simple_ner(text):
...
# === CLEAN WORKING CoNLL PARSER → HF DATASET ===
from pathlib import Path
...
print("✅ Week 6: NER and summarization models loaded.")
...
def extract_entities(text: str) -> List[str]:
    ...
    for tok, tag_id in zip(tokens, pred_ids):
        ...
        if prefix == "B":
            ...
            # inconsistent tag – close old entity, start new
            if current_tokens:
                entities.append(" ".join(current_tokens))
            current_tokens = [clean_token(tok)]
            current_type = ent_type
...
def make_safe_fallback_summary(source: str, covered_entities: List[str]) -> str:
    """
    Very conservative backup summary – only uses entities known to be in the source.
    """
...
def summarize_with_mitigation(text: str, max_regenerations: int = 1) -> Dict:
    ...
    # Still hallucinating → fallback safe template
...
print("✅ Week 6 code (verification + mitigation) is ready.")
...
print("✅ Week 7: post-processing functions ready.")
...
print("✅ Week 7: automatic evaluation ready.")
